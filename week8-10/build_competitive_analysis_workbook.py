...
DEFAULT_PUBLISH_COPY = PROJECT_DIR / "week8-10" / "competitive_analysis.twbx"
...
# The overview needs a compact summary. Reusing the 100-row worksheets in a
# half-height dashboard forces Tableau to squeeze the row labels together, so
# the overview uses dedicated Top 25 copies while the required dashboards keep
# the complete dynamic Top 100 rankings.
OVERVIEW_RANKING_SHEETS = [
    {
        **RANKING_SHEETS[0],
        "name": "Top 25 Listing Agents - Sales Volume (Overview)",
        "display_title": "Top 25 Listing Agents - Sales Volume",
        "limit": 25,
    },
    {
        **RANKING_SHEETS[2],
        "name": "Top 25 Listing Offices - Sales Volume (Overview)",
        "display_title": "Top 25 Listing Offices - Sales Volume",
        "limit": 25,
    },
]
...
ALL_SHEETS = RANKING_SHEETS + OVERVIEW_RANKING_SHEETS + MAP_SHEETS
DASHBOARDS = [
...
def ranking_worksheet_xml(sheet: dict[str, str]) -> str:
    ...
    limit = sheet.get("limit", 100)
    display_title = sheet.get("display_title", sheet["name"])
    top_filter = f"""          <filter class='categorical' column='[{DATASOURCE}].[{dimension_instance}]'>
            <groupfilter count='{limit}' end='top' function='end' units='records' user:ui-marker='end' user:ui-top-by-field='true'>
    ...
    return f"""    <worksheet name='{xml_escape(sheet['name'])}'>
      <layout-options><title><formatted-text><run fontname='Tableau Medium' fontsize='11'>{xml_escape(display_title)}</run></formatted-text></title></layout-options>
      <table>
        <view>
...
def map_worksheet_xml(sheet: dict[str, str]) -> str:
    ...
    filters, slices = shared_filters_xml()
    return f"""    <worksheet name='{xml_escape(sheet['name'])}'>
      <layout-options><title><formatted-text><run fontname='Tableau Medium' fontsize='11'>{xml_escape(sheet['name'])}</run></formatted-text></title></layout-options>
      <table>
        <view>
          <datasources><datasource caption='Week 8 Competitive Sales' name='{DATASOURCE}' /></datasources>
          <datasource-dependencies datasource='{DATASOURCE}'>
...
{slices}
        ...
        <panes>
          <pane selection-relaxation-option='selection-relaxation-disallow'>
            ...
            <style><style-rule element='mark'><format attr='size' value='0.35' /></style-rule></style>
            <customized-tooltip>
...
def pair_dashboard_xml(name: str, title: str, left_sheet: str, right_sheet: str) -> str:
    ...
    return f"""    <dashboard enable-sort-zone-taborder='true' name='{xml_escape(name)}'>
      <layout-options><title><formatted-text><run fontname='Tableau Semibold' fontsize='15'>{xml_escape(title)}</run></formatted-text></title></layout-options>
      <style />
...
def overview_dashboard_xml() -> str:
    anchor = OVERVIEW_RANKING_SHEETS[0]["name"]
    zones = [
        "        <zone h='100000' id='100' type-v2='layout-basic' w='100000' x='0' y='0'>",
        "          <zone h='6500' id='101' type-v2='title' w='100000' x='0' y='0' />",
        sheet_zone_xml(OVERVIEW_RANKING_SHEETS[0]["name"], 1, 0, 7000, 40500, 45500),
        sheet_zone_xml(OVERVIEW_RANKING_SHEETS[1]["name"], 2, 40500, 7000, 40500, 45500),
...
def windows_xml() -> str:
    ...
    viewpoints = {
        ...
        "Competitive Overview": [OVERVIEW_RANKING_SHEETS[0]["name"], OVERVIEW_RANKING_SHEETS[1]["name"], MAP_SHEETS[0]["name"], MAP_SHEETS[1]["name"]],
    }
...
def workbook_xml() -> str:
    ...
    worksheets += "\n" + "\n".join(
        ranking_worksheet_xml(sheet) for sheet in OVERVIEW_RANKING_SHEETS
    )
...
def build(csv_path: Path, output_path: Path, reuse_hyper: bool = False) -> tuple[Path, Path]:
    hyper_path = output_path.with_suffix(".hyper")
    if reuse_hyper:
        if not hyper_path.exists():
            raise FileNotFoundError(f"Existing Hyper extract not found: {hyper_path}")
        row_count = None
    else:
    ...
    if row_count is None:
        print(f"Reused existing extract: {hyper_path}")
    else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "defaults to week8-10/competitive_analysis.twbx"
        ),
    )
    parser.add_argument(
        "--reuse-hyper",
        action="store_true",
        help="reuse the existing packaged Hyper extract and only rebuild workbook formatting",
    )
...
def main() -> None:
    args = parse_args()
    twb_path, twbx_path = build(
        args.csv.resolve(), args.output.resolve(), reuse_hyper=args.reuse_hyper
    )
    ...
    print("Worksheets: 8 (6 required + 2 compact overview summaries)")
